...
const initClick = () => {
  ...
  const handleClickCard = () => {
    ...
    // 卡片点击事件抛出
    emits('cardClick', { key: key.value, detailVisible: detailVisible})
...
const initHover = () => {
  ...
  const handleMouseOverCard = () => {
    ...
    if (hoverStatus.value === 'ready' || hoverStatus.value === 'complete') {
      // 鼠标悬浮时抛出方法
      emits('cardReadyOver', key.value)
      handleAddHoverStyle()
      hoverStatus.value = 'activity'
      const hover = containerRef.value.animatuje([
  ...
  const handleMouseLeaveCard = () => {
    if (showStatus.value === 'ready') {
      const hover = containerRef.value.animate([
        { transform: 'scale(1)' }
      ], {
        duration: 100,
        easing: 'ease-in-out',
      ...
      hover.onfinish = () => {
        ...
        // 鼠标悬浮结束后抛出
        emits('cardReadyLeave', key.value)
      }
      // gsap.to('.' + key.value, {
      //   scale: 1,
      //   duration: 0.1,
