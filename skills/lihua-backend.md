...
- Maven 父工程：本仓根 `pom.xml`，`<modules>` 为 `lihua-admin`、`lihua-base`、`lihua-biz`；编译版本 `java.version=25`（README 的 Java 版本描述仅是最低环境参考）。
...
- 基础模块（`lihua-base/`，14 个）：attachment、cache、captcha、common、dict、doc、excel、job、log、mybatis、security、sensitive、web、websocket。
...
4. 管理端写操作一律 `@PreAuthorize`（现有口径 `hasRole('ROLE_admin')` 粗粒度；细粒度 authorities 通道保留但不消费）+ `@Log(description=..., type=LogTypeEnum.*)`，密码等敏感参数排除出日志。操作日志用 `lihua-base-log` 注解能力，不临时写审计逻辑；实时通知投递用 `WebSocketPushUtils`（base-cache，Redis pub/sub 扇出，勿直调 WebSocketManager）——base-websocket（连接层）由 lihua-admin 装配引入，业务模块 pom 零依赖；安全判断用基础安全模块，不手写认证判断。
...
- **WS 部署形态**：mono 单 jar WS 嵌 admin 进程（base-websocket 由 lihua-admin 显式引入，业务模块零依赖；推送投递经 Redis pub/sub 回本进程订阅器推送，mono 部署多份天然扇出）/ cloud 独立 `lihua-ws` 服务（无库、可多实例；gateway 路由 `/ws-connect/**` 指向 lihua-ws；system 已卸除 base-websocket）。两形态同构：base-websocket（连接持有方）与业务模块完全隔离、仅经 Redis 交互（WS_PUSH 投递/订阅 + once token 握手鉴权读写）；业务侧投递一律 `WebSocketPushUtils.push`（base-cache），禁止依赖 base-websocket/直调 WebSocketManager。
